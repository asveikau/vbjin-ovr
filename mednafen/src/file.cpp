...
bool MDFNFILE::ApplyIPS(MDFNFILE *ips)
{
 ...
 while(ips->fread(header, 1, 3) == 3)
 {
  ...
  patch_size = ips->fgetc() << 8;
  patch_size |= ips->fgetc();
  if(!patch_size)	/* RLE */
  {
   uint8 *start;
   uint8 b;
   ...
   if((offset + patch_size) > f_size)
   {
    ...
    tmp=(uint8 *)MDFN_realloc(f_data, offset + patch_size, _("file read buffer"));
    if(!tmp)
    {
     return(0);
    }
    ...
    // FIXME!!!!
    memset(f_data + f_size, 0, offset + patch_size - f_size);
   }
  ...
  else		/* Normal patch */
  {
   ...
   if((offset + patch_size) > f_size)
   {
    ...
    f_data=tmp;
    memset(f_data + f_size, 0, offset + size - f_size);
   }
   ips->fread(f_data + offset, 1, size);
  }
  count++;
 }
